...
def run_showcase():
    ...
    prop = AIProposal(
        id="AI-042",
        strategy="Quad-symmetry with staged decouplers and aerodynamic shielding.",
        expected_benefit="Perfect balance",
    )
    dec = HumanDecision(
        status=DecisionStatus.ACCEPT,
        reason="Illustrative approval for this synthetic demo",
    )
    best_exp = Experiment(
        id="EXP-042",
        build_mode="Copilot",
        hypothesis="Staging works",
        proposal=prop,
        human_decision=dec,
    )
    ...
    exp2 = Experiment(
        ...
        hypothesis="Compiled Run",
        proposal=prop,
        human_decision=dec,
    )
    exp2.result = ExperimentResult(
        0.95, 0.88, 1.0, 94.1, 0.5, 107.6
    )  # x1.15 and lower penalty!
