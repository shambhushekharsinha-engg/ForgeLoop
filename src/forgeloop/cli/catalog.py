...
from ..public_sources import DEFAULT_CATALOG, load_catalog, refresh_catalog, search_blocks
...
def main(argv=None):
    ...
    parser.add_argument("--refresh", type=Path, metavar="DESTINATION",
                        help="Download pinned public data into a cache; no key needed")
    parser.add_argument("--json", action="store_true", help="Include provenance in JSON output")
    args = parser.parse_args(argv)
    try:
        catalog = refresh_catalog(args.refresh) if args.refresh else load_catalog(args.catalog)
    ...
    if args.json:
        ...
        print(f"{len(matches)} matching blocks (public metadata; no physics validation)")
