...
def _dependency_check(name):
    ...
    return {
        ...
        'required': False,
        'detail': f'{version}; installation detected, runtime import not tested' if found else
...
def check_environment(catalog_path=DEFAULT_CATALOG):
    ...
    checks = [{
        'name': 'python', 'status': 'available' if python_ok else 'unsupported', 'required': True,
        'detail': f'{platform.python_version()} (requires 3.10 or newer)',
    }]
    try:
        catalog = load_catalog(catalog_path)
        checks.append({'name': 'catalog', 'status': 'available', 'required': True,
                       'detail': f"{len(catalog['blocks'])} blocks; pinned revision {catalog['revision']}"})
    except (OSError, ValueError) as error:
        checks.append({'name': 'catalog', 'status': 'invalid', 'required': True, 'detail': str(error)})
    ...
    return {
        ...
        'core_ready': all(check['status'] == 'available' for check in checks if check['required']),
        'analysis_available': all(check['status'] == 'available' for check in dependencies),
        'checks': checks,
...
def main(argv=None):
    ...
    parser.add_argument('--catalog', type=Path, default=DEFAULT_CATALOG, help='Local catalog to validate')
    parser.add_argument('--json', action='store_true', help='Emit machine-readable readiness report')
    parser.add_argument('--require-analysis', action='store_true', help='Fail if optional analysis dependencies are absent')
    ...
    return 0 if report['core_ready'] and (not args.require_analysis or report['analysis_available']) else 1
