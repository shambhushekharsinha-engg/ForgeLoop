...
def analyze(args):
    ...
    try:
        ...
        raise ValueError('Install analysis dependencies: pip install ".[analysis]"') from error
    import pandas as pd
    ...
    from ..decisions.decision import HumanDecision, DecisionStatus
    ...
    speed = calculate_speed_score(trajectory, orbit, reference_duration=args.reference_duration)
    integrity = calculate_integrity(trajectory)
    exp = Experiment(
        id=args.id, build_mode=args.build_mode, hypothesis=args.hypothesis,
        proposal=AIProposal(id=args.id + '-proposal', strategy=args.strategy, expected_benefit=args.hypothesis),
        human_decision=HumanDecision(status=DecisionStatus(args.decision), reason=args.reason),
    ...
    record = ExperimentStore(args.store).save(
        exp, csv_bytes, transcript_bytes, evidence_kind=args.evidence_kind,
        assumptions={'format': 'forgeloop-normalized-v1', 'target_degrees': 1080,
...
def main(argv=None):
    ...
    analysis = commands.add_parser('analyze', help='Evaluate normalized CSV and save original evidence')
    ...
    analysis.add_argument('--decision', choices=['ACCEPT', 'REJECT'], required=True,
    ...
    analysis.add_argument('--build-mode', choices=['Copilot', 'Autopilot'], required=True)
    analysis.add_argument('--evidence-kind', choices=['measured', 'synthetic'], required=True)
    analysis.add_argument('--reference-duration', type=float, required=True, help='Explicit local speed baseline in seconds')
    analysis.add_argument('--store', type=Path, default=Path('runs/experiments.sqlite3'))
    listing = commands.add_parser('list', help='Inspect saved experiments and verify evidence hashes')
    ...
    dashboard = commands.add_parser('dashboard', help='Export saved results for one evidence category')
    dashboard.add_argument('--store', type=Path, default=Path('runs/experiments.sqlite3'))
    dashboard.add_argument('--evidence-kind', choices=['measured', 'synthetic'], required=True)
    ...
    try:
        if args.command == 'analyze':
            ...
            print(json.dumps(ExperimentStore(args.store).records(args.evidence_kind), indent=2, allow_nan=False))
        elif args.command == 'dashboard':
            from .export_web import export_dashboard
            store = ExperimentStore(args.store)
            if args.output.resolve() == store.path or args.output.resolve() in store.path.parents:
                raise ValueError('Dashboard output must be separate from the evidence database')
            registry = store.registry(args.evidence_kind)
            output = export_dashboard(registry, output_dir=args.output, plots_dir=args.output / 'plots')
            # The exporter escapes all registry values. Make the category explicit.
            page = output.read_text(encoding='utf-8').replace('Local Experiment Dashboard', f'{args.evidence_kind.title()} Inputs — Local Estimates')
            page = page.replace('No evaluated experiments supplied.', f'No {args.evidence_kind} experiments supplied.')
            page = page.replace('<body ', '<body data-evidence-kind="' + args.evidence_kind + '" ', 1)
            # Visible label alongside the page heading, without modifying layout.
            page = page.replace('ForgeLoop<span', args.evidence_kind.title() + ' Inputs · ForgeLoop<span', 1)
