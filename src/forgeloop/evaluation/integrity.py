def calculate_integrity(trajectory) -> float:
    ...
    if not required.issubset(trajectory.data.columns):
        raise ValueError(
            "Integrity requires connected_blocks and tracked_blocks measurements"
        )
    ...
    if (
        not ((connected >= 0) & (connected <= initial)).all()
        or not connected.map(lambda v: float(v).is_integer()).all()
    ):
        raise ValueError(
            "connected_blocks must be integer counts within the initial population"
        )
