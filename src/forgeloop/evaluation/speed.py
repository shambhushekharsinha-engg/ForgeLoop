...
def calculate_speed_score(
    trajectory, orbit_progress, *, reference_duration=None
) -> float:
    ...
    if (
        ...
        or not math.isfinite(reference_duration)
        or reference_duration <= 0
    ):
        raise ValueError(
            "Speed estimation requires a finite positive reference_duration in seconds"
        )
