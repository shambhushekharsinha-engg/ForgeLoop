...
def _text(value, field, limit=1024):
    if (not isinstance(value, str) or not value.strip() or len(value) > limit
        ...
        raise ValueError(f"Invalid catalog {field}: expected bounded nonempty text without control characters")
...
def _validate_blocks(blocks):
    ...
    for block in blocks:
        ...
        if type(block_id) is not int or not 0 <= block_id <= 2**31 - 1 or block_id in seen:
            raise ValueError("Invalid catalog block ID: IDs must be unique nonnegative integers")
        seen.add(block_id)
        ...
        if 'pointer_axis' in block:
            ...
            if (not isinstance(axis, list) or len(axis) != 3
                    or any(type(value) not in (int, float) or abs(value) > 1e12 or not math.isfinite(value) for value in axis)):
                raise ValueError("Invalid catalog pointer_axis: expected three finite numbers")
...
def validate_catalog(catalog: dict) -> dict:
    ...
    if not isinstance(catalog, dict):
        raise ValueError("Invalid catalog: expected an object")
    version = catalog.get('schema_version', 1)
    if type(version) is not int or version != SCHEMA_VERSION:
        raise ValueError("Unsupported catalog schema_version")
    for field, expected in [('source', SOURCE_URL), ('revision', REVISION),
                            ('license', 'CC-BY-NC-4.0'), ('license_url', LICENSE_URL)]:
        if catalog.get(field) != expected:
            raise ValueError(f"Invalid catalog provenance: {field} does not match the pinned source")
    ...
    try:
        retrieved = datetime.fromisoformat(catalog['retrieved_at'].replace('Z', '+00:00'))
...
def parse_roles(payload: bytes) -> list[dict]:
    ...
    for block_id, role in roles.items():
        if not re.fullmatch(r'[0-9]{1,10}', str(block_id)) or not isinstance(role, dict):
            raise ValueError("Invalid block role")
        block = {"id": int(block_id), "name": role.get("block_name"), "type": role.get("type")}
...
def refresh_catalog(destination: Path) -> dict:
    ...
    """
    request = Request(SOURCE_URL, headers={"User-Agent": "ForgeLoop-public-catalog/1.0"})
...
def search_blocks(catalog: dict, query: str = "", kind: str | None = None) -> list[dict]:
    validate_catalog(catalog)
    if len(query) > 1024 or (kind is not None and len(kind) > 64):
        raise ValueError("Catalog query exceeds size limit")
    terms = query.casefold().split()
    return [block for block in catalog["blocks"]
            if (kind is None or block["type"].casefold() == kind.casefold())
            and all(term in f"{block['id']} {block['name']} {block['type']}".casefold()
                    for term in terms)]
