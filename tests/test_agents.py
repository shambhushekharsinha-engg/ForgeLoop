...
def experiment(status=DecisionStatus.ACCEPT, modified=None, result=True):
    return SimpleNamespace(
        id="EXP-001",
        proposal=SimpleNamespace(strategy="Original strategy"),
...
def test_compiler_uses_reviewed_modification_and_does_not_claim_success():
    ...
    prompt = compiler.compile_megaprompt(
        experiment(DecisionStatus.MODIFY, "Corrected strategy")
    )
...
def test_strategist_describes_measurements_without_diagnosing_physics():
    ...
    frame = pd.DataFrame(
        {"z": [100, 0], "angular_progress": [500, 550], "vz": [-50, 50]}
    )
...
def test_strategist_reports_missing_and_invalid_data():
    ...
    assert "Missing telemetry fields" in strategist.analyze_flight(
        pd.DataFrame({"z": [1]})
    )
