...
def test_doctor_reports_invalid_catalog_with_failure_status(tmp_path, capsys):
    ...
    assert next(check for check in report['checks'] if check['name'] == 'catalog')['status'] == 'invalid'
...
def test_detected_dependencies_are_not_treated_as_import_verified(monkeypatch):
    ...
    assert all('runtime import not tested' in check['detail'] for check in report['checks'] if not check['required'])
