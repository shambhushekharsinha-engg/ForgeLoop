...
@pytest.mark.parametrize(
    "performance,cost,mode",
    [
        (101, 0, "Copilot"),
        (50, -1, "Copilot"),
        (float("nan"), 0, "Copilot"),
        (50, float("inf"), "Copilot"),
        (50, 0, "Autopliot"),
    ],
)
...
def test_orbit_accounts_for_start_angle_and_direction(tmp_path, angles):
    ...
    trajectory = write_trajectory(
        tmp_path,
        f"timestamp,x,y,z,angular_progress\n0,1,0,1,{a}\n60,0,1,1,{b}\n120,1,0,1,{c}\n",
    )
...
def test_oscillation_does_not_accumulate_orbits(tmp_path):
    ...
    trajectory = write_trajectory(
        tmp_path,
        "timestamp,x,y,z,angular_progress\n0,1,0,1,0\n1,0,1,1,180\n2,1,0,1,0\n3,0,1,1,180\n",
    )
...
def test_integrity_uses_original_population(tmp_path):
    ...
    trajectory = write_trajectory(
        tmp_path,
        "timestamp,x,y,z,tracked_blocks,connected_blocks\n0,1,2,3,10,10\n1,2,3,4,10,7\n",
    )
...
def test_mutated_invalid_trajectory_is_rejected_by_evaluators(tmp_path):
    ...
    trajectory = write_trajectory(
        tmp_path, "timestamp,x,y,z,angular_progress\n0,1,0,1,0\n1,0,1,1,360\n"
    )
...
def test_invalid_integrity_counts_rejected(tmp_path, tracked, connected):
    ...
    trajectory = write_trajectory(
        tmp_path,
        f"timestamp,x,y,z,tracked_blocks,connected_blocks\n0,1,2,3,{tracked},{tracked}\n1,2,3,4,{tracked},{connected}\n",
    )
    with pytest.raises(ValueError):
        calculate_integrity(trajectory)
