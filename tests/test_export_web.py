...
def test_empty_export_does_not_fabricate_results_or_flight_evidence(tmp_path):
    output = export_dashboard(output_dir=tmp_path / 'site', plots_dir=tmp_path / 'missing')
...
def test_export_renders_only_supplied_results_sorted_and_escaped(tmp_path):
    def experiment(name, score):
        return SimpleNamespace(
            id=name, hypothesis='<script>alert(1)</script>', build_mode='Copilot',
            result=SimpleNamespace(final_score=score, orbit_progress=.5, cost_penalty=2),
        )
    registry = SimpleNamespace(experiments={
        'low': experiment('LOW', 10), 'high': experiment('HIGH', 20),
...
def test_writeup_marks_unknown_details_and_preserves_prompt_fence(tmp_path):
    ...
    experiment = SimpleNamespace(
        id='EXP-001', hypothesis='Balance', build_mode='Copilot', notes='', result=None,
    )
    target = tmp_path / 'nested' / 'writeup.md'
    prompt = 'Build this\n```\nuntrusted fence'
    assert WriteupGenerator(target).generate('Test machine', experiment, prompt) == target
