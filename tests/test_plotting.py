...
def test_headless_png_uses_measured_coordinates_without_invented_planet(tmp_path, plot_modules, monkeypatch):
    ...
    result = Plotter(tmp_path).plot_orbit('EXP-001', pd.DataFrame({'x': [1, 2], 'y': [3, 4], 'z': [5, 6]}))
...
def test_explicit_center_and_save_failure_cleanup(tmp_path, plot_modules, monkeypatch):
    ...
    with pytest.raises(OSError, match='disk full'):
        Plotter(tmp_path).plot_orbit('EXP-001', pd.DataFrame({'x': [1, 2], 'y': [3, 4], 'z': [5, 6]}), center=(10, 20, 30))
...
def test_plot_path_traversal_rejected(tmp_path, plot_modules, identifier):
    ...
    with pytest.raises(ValueError):
        Plotter(tmp_path).plot_orbit(identifier, pd.DataFrame({'x': [1], 'y': [2], 'z': [3]}))
