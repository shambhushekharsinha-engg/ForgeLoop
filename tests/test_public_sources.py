...
def test_refresh_records_provenance_and_preserves_cache_on_failure(
    tmp_path, monkeypatch
):
    destination = tmp_path / "catalog.json"
    monkeypatch.setattr(
        public_sources, "urlopen", lambda *args, **kwargs: io.BytesIO(PAYLOAD)
    )
    ...
    monkeypatch.setattr(
        public_sources, "urlopen", lambda *args, **kwargs: io.BytesIO(b"bad TOML")
    )
    with pytest.raises(ValueError):
...
def test_concurrent_refreshes_use_independent_atomic_temporary_files(
    tmp_path, monkeypatch
):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        futures = [
            pool.submit(public_sources.refresh_catalog, destination) for _ in range(2)
        ]
...
def test_failed_replace_cleans_temporary_file_and_preserves_cache(
    tmp_path, monkeypatch
):
    destination = tmp_path / "catalog.json"
