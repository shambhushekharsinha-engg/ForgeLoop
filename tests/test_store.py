...
def experiment(name="EXP-001"):
    exp = Experiment(
        ...
        AIProposal("P1", "Test", "Test"),
        HumanDecision(DecisionStatus.ACCEPT, "Test only"),
    )
    ExperimentRunner().evaluate_experiment(exp, 0.5, 0.5, 1, "test transcript")
    return exp
...
def test_saved_experiments_survive_reopening_and_keep_evidence(tmp_path):
    ...
    with sqlite3.connect(path) as connection:
        assert (
            connection.execute("SELECT transcript FROM experiments").fetchone()[0]
            == b"test transcript"
        )
...
def test_analyze_real_pipeline_and_duplicate_protection(tmp_path, capsys):
    ...
    args = [
        "analyze",
        str(root / "examples/synthetic-flight.csv"),
        "--transcript",
        str(root / "examples/synthetic-transcript.md"),
        "--id",
        "EXP-001",
        "--hypothesis",
        "Pipeline",
    ...
    assert record["result"]["integrity"] == 0.9
