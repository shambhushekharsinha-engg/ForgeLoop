...
from forgeloop.data.validators import validate_bsg_structure, validate_no_geometry_changes
...
def test_invalid_evidence_preserves_previous_archive(artifacts, tmp_path, name, content):
    source, _ = artifacts
    (source / name).write_bytes(content)
    output = tmp_path / "submission.zip"
...
def test_output_cannot_overwrite_evidence(artifacts):
    ...
    assert (source / "chat_transcript.md").read_bytes() == contents["chat_transcript.md"]
...
def test_scrubber_only_redacts_explicit_tool_output():
    ...
    scrubber = TranscriptScrubber()
    assert scrubber.scrub(transcript) == transcript
    assert scrubber.scrub(transcript, redact_tool_outputs=True) == 'Human: keep me\n[TOOL_OUTPUT_REDACTED_IN_REVIEW_COPY]\nAI: decision\n'
...
@pytest.mark.parametrize("content", [
    ...
    BSG.replace(b'<Machine ', b'<!DOCTYPE Machine [<!ENTITY a "payload">]><Machine ', 1),
])
...
def test_archive_created_during_validation_is_not_clobbered(artifacts, tmp_path, monkeypatch):
    source, _ = artifacts
    output = tmp_path / "submission.zip"
